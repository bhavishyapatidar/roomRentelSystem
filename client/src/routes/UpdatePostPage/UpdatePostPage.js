...
function UpdatePostPage() {
  ...
  useEffect(() => {
    const fetchPostData = async () => {
      try {
        ...
        setImages(res.data.images); // Assuming images are an array
      } catch (err) {
        console.log(err);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/${id}`); // Redirect to the updated post page
    } catch (err) {
        console.log(err);
  ...
  if (!postData) return <p>Loading...</p>; // Show loading while fetching post data
return (
